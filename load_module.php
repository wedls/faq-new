...
$module = isset($_GET['module']) ? intval($_GET['module']) : 1;
$questions = getQuestions($module);
$instruction = getInstruction($module);
$total_questions = count($questions);
$current_responses = isset($_SESSION['responses'][$module]) ? $_SESSION['responses'][$module] : [];

// Определяем, сколько вопросов показывать за раз (для производительности)
...
$current_questions = array_slice($questions, $start_index, $questions_per_page, true);
?>

<div class="module-content" data-module="<?php echo $module; ?>" data-page="<?php echo $page; ?>" data-total-pages="<?php echo ceil($total_questions / $questions_per_page); ?>">
    <h2 class="module-title">

        <span class="module-content-b-t">
            <span class="module-content-b-t-t"><?=$module_titles[$module]['title']?></span>
            <span class="module-content-b-t-d"><?=$module_titles[$module]['description']?></span>
        </span>
        <span class="module-content-b-i">
            <?php switch ($module): ?><?php case 1: ?>
    ...
    <div class="cont-question">
        <?php 
        $question_counter = $start_index;
        $total_current_questions = count($current_questions);
        $current_index = 0;
        
        foreach ($current_questions as $q_num => $question_text): 
            // Для корректной нумерации используем ключи из массива
            // Для модуля 2, часть 1: вопросы 1-60, часть 2: вопросы 61-95
            $absolute_q_num = $q_num;
            
            // Определяем правильные варианты ответов для этого вопроса
            $answer_options = getAnswerOptions($module, $absolute_q_num);
            
            // Определяем классы для первого и последнего вопроса
            $container_classes = 'question-container';
            if ($current_index === 0) {
                $container_classes .= ' question-container-first';
            }
            if ($current_index === $total_current_questions - 1) {
                $container_classes .= ' question-container-last';
            }
        ?>
        
            <div class="<?php echo $container_classes; ?>" data-question="<?php echo $absolute_q_num; ?>">
                <div class="question-header">
                    <div class="question-number"><?php echo $absolute_q_num; ?></div>
                    <div class="question-text"><?php echo $question_text; ?></div>
                </div>
                <div class="options-container">
                    <?php 
                    foreach ($answer_options as $value => $text):
                        $selected = (isset($current_responses[$absolute_q_num]) && $current_responses[$absolute_q_num] == $value) ? 'selected' : '';
                    ?>
                    <button class="option-btn <?php echo $selected; ?>" 
                            data-value="<?php echo htmlspecialchars($value); ?>"
                            data-question="<?php echo $absolute_q_num; ?>">
